...
import { ProductStore } from "./catalog";
...
test("associated services exclude managed, nested, and unrelated paths", () => {
  ...
  const initial = discovery.observe(repo);
  expect(initial.worktrees[0]?.services.map((item) => item.pid)).toEqual([
    1001,
  ]);
  expect(probed).toEqual([1001]);
  expect(store.events()).toEqual([]);
  // The application explicitly records successful observations; discovery is read-only.
  store.observeDetected(initial);
  services = [];
  warning = "Inspection unavailable";
  discovery.observe(repo);
  expect(store.events()).toEqual([]);
  warning = null;
  store.observeDetected(discovery.observe(repo));
